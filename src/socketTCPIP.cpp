...
socketTCPIP::socketTCPIP(unsigned int port, char connectionAddress[32]) {
...
PcPackage socketTCPIP::receiveData() {
	// Aguarda o recebimento de dados do PC
	int ret;
	PcPackage pcPackage;

	ret = recv(sdHost, &pcPackage, sizeof(PcPackage), 0);
	if (ret <= 0) {
...
int socketTCPIP::sendData(BeaglePackage beaglePackage) {
	// Envia dados ao PC
	int ret;

	ret = send(sdHost, &beaglePackage, sizeof(BeaglePackage), 0);
	if (ret == -1) {
		printf("error\n");
		return -1;
	}
